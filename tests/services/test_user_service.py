...
def user_service(
    ...
    """Provide a user user_service with mocked persistence dependencies."""

    return UserService(repository=mock_user_repository, session=mock_database_session)
...
async def test_register_creates_user_with_hashed_password(
    ...
    mock_user_repository.add.side_effect = lambda user: user
    ...
    added_user = mock_user_repository.add.await_args.args[0]
    ...
    mock_user_repository.add.assert_awaited_once_with(added_user)
    mock_database_session.commit.assert_awaited_once_with()
    mock_database_session.rollback.assert_not_awaited()
...
async def test_register_rejects_existing_username(
    ...
    mock_user_repository.get_by_email.assert_not_awaited()
...
async def test_register_rolls_back_when_database_rejects_user(
    ...
    mock_user_repository.get_by_username.return_value = None
    mock_user_repository.get_by_email.return_value = None
    mock_user_repository.add.side_effect = IntegrityError(
        statement=None,
        params=None,
    ...
    mock_database_session.commit.assert_not_awaited()
    mock_database_session.rollback.assert_awaited_once_with()
...
async def test_register_propagates_unexpected_repository_errors(
    ...
    mock_user_repository.get_by_username.return_value = None
    mock_user_repository.get_by_email.return_value = None
    mock_user_repository.add.side_effect = RuntimeError("Unexpected failure")
    ...
    mock_user_repository.get_by_username.assert_awaited_once_with(user_create.username)
    mock_user_repository.get_by_email.assert_awaited_once_with(user_create.email)
